updating_last_subculture <- tabPanel("Updating Last Subculture", value = "updating_last_subculture",
                              ...
                              div(id = "updating_last_subculture_Form",       
                                     ...
                                     fluidRow(
                                       column(10,offset = 1,
                                         column(4,
                                                ...
                                                disabled(selectInput('updating_last_subculture_CulturedBy', "Cultured By", choices = c(""), width = "100%")),
                                                disabled(selectInput("updating_last_subculture_Media", "Media", choices = c(""), width = "100%")),
                                     ...
                                     fluidRow(
                                       column(10, offset = 1,
                                         column(2, offset = 2, actionBttn("updating_last_subculture_Update", "Update", style = "material-flat", size = "xs", color = "primary")),
                                         column(2),
                                         column(2, actionBttn("updating_last_subculture_Clear", "Clear", style = "material-flat", size = "xs", color = "primary"))
                                     )),
                                     fluidRow(br(),hr(),
                                              column(8, offset = 2,
                                                     column(2, actionBttn("updating_last_subculture_FormToPicture", "Form to Picture", style = "jelly", size = "xs", color = "success", block=T)),
                                                     column(4, actionBttn("updating_last_subculture_MFC_SCP_CSC_ControlForm", "MFC, SCP and CSC Control Form", style = "jelly", size = "xs", color = "warning", block=T)),
                                                     column(2, tags$button(id = 'updating_last_subculture_Exit', type = "button", class = "btn action-button",
                                                                           onclick = "setTimeout(function(){window.close();},500);",  # close browser
                                                                           "Exit", style="color: #fff; background-color: #ff0011; border-color: #ff0011; padding:4px; font-size:80%;")
                                                            #actionBttn("updating_last_subculture_Exit", "Exit", style = "jelly", size = "xs", color = "danger", block=T)
                                                            )
